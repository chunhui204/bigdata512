...
/* this function should be called after an i2c-command has been written. 
   It reports a timeout (-1) or the error-bits set like in described in the specs,
   p.123, table 110 */
int i2c_busy_rise_and_fall(struct saa7146* saa, int timeout)
{
	...
	/* on some machines the first status check returns garbage(?) */
	...
	hprintk("saa7146: i2c_busy_rise_and_fall: count %d\n", timeout-i);
