...
/* insmod parameter: allows you to specify the pci latency timer.
   Value 0 (default) means 'do not change'. */
...
int configure_saa7146 (struct saa7146 *saa)
{
	...
	if (latency) {
		printk(KERN_INFO "saa7146_core.o: adjusting pci latency to %d\n", latency);
		pci_write_config_byte(saa->device, PCI_LATENCY_TIMER, (unsigned char)latency);
	}
...
static struct card_info ttbt2  = { DVB_CARD_TT_BUDGET,    "TT-Budget/WinTV-NOVA-T  PCI" };
...
static struct pci_device_id saa7146_pci_tbl[] __devinitdata = {
	...
	{ PHILIPS_SAA7146, 0x13c2, 0x1011, CARD_INFO ttbt2 },
